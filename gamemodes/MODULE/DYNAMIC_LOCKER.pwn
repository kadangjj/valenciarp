...
CMD:createlocker(playerid, params[])
{
	...
	new type;
	if(sscanf(params, "d", type)) return SendClientMessageEx(playerid, COLOR_WHITE, "[USAGE]: /createlocker [type,  1.PD 2.GS 3.MD 4.NA]");
...
CMD:editlocker(playerid, params[])
{
    ...
    else if(!strcmp(type, "type", true))
    {
        ...
        if(sscanf(string, "d", tipe))
            return SendClientMessageEx(playerid, COLOR_WHITE, "[USAGE]: /editlocker [id] [type] [type, 1.PD 2.GS 3.MD 4.NA]");

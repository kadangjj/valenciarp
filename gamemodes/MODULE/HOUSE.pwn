...
GetHouseStorage(houseid, item)
{
	static const StorageLimit[][] = {
	   //Snack  Sprunk  Medicine  Medkit  Bandage  Seed  Material  Component  Marijuana
		...
		{75,    75,     50,  	  30,  	  50,  	   450,  450,  	   500, 	 300},// Large
...
CMD:edithouse(playerid, params[])
{
    ...
    else if(!strcmp(type, "price", true))
	{
		...
		// Set harga rumah
		hData[hid][hPrice] = strval(totalcash);

		// Update harga rumah di database
		new query[128];
		mysql_format(g_SQL, query, sizeof(query), "UPDATE houses SET price='%d' WHERE ID='%d'", strval(totalcash), hid);
		mysql_tquery(g_SQL, query);

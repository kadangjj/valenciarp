...
#if !defined MAX_DOOR
    #define MAX_DOOR 5000
#endif

// Deklarasi dengan +1 untuk YSI v4
enum ddoor
{
	dName[128],
	dPass[32],
...
new Iterator:Doors<MAX_DOOR + 1>; // TAMBAH +1 INI PENTING untuk YSI v4
...
function LoadDoors()
{
    ...
    if(rows == 0)
    {
        print("[Doors]: No doors found in database.");
        return 1;
    }
    
    printf("[Doors]: Loading %d doors from database...", rows);
    
    new did, name[128], password[128];
    new loaded = 0, skipped = 0;
    ...
    for(new i = 0; i < rows; i++)
    {
        ...
        {
            printf("[ERROR] Invalid door ID: %d (Max: %d) - Skipped", did, MAX_DOOR - 1);
            skipped++;
            continue;
        }
        
        // Cek duplikat
        if(Iter_Contains(Doors, did))
        {
            printf("[WARNING] Duplicate door ID: %d - Skipped", did);
            skipped++;
            continue;
        }
        
        // Load semua data dulu SEBELUM Iter_Add
        ...
        // Tambahkan ke iterator SETELAH semua data loaded
        Iter_Add(Doors, did);
    ...
    printf("[Doors]: %d loaded successfully, %d skipped.", loaded, skipped);
    return 1;
}
...
CMD:createdoor(playerid, params[])
{
	...
	// VALIDASI TAMBAHAN
	if(did == -1 || did >= MAX_DOOR) 
		return Error(playerid, "You cant create more door!");
	
	new name[128];
	if(sscanf(params, "s[128]", name)) 
		return SendClientMessageEx(playerid, COLOR_WHITE, "[USAGE]: /createdoor [name]");
...
CMD:gotodoor(playerid, params[])
{
	...
	if(!Iter_Contains(Doors, did)) 
		return Error(playerid, "The doors you specified ID of doesn't exist.");

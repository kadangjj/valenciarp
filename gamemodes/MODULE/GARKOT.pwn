...
// Fixed: Added proper comparison and consistent variable naming
GetClosestParks(playerid, Float: range = 4.3)
{
	new id = -1, Float: dist = range, Float: tempdist;
	foreach(new i : Parks)
	{
		if(GetPlayerInterior(playerid) != ppData[i][parkInt] || GetPlayerVirtualWorld(playerid) != ppData[i][parkWorld])
			continue;
...
// Fixed: String format moved after data loaded
function LoadPark()
{
	new rows;
	...
	if(rows)
	{
		...
		while(i < rows)
		{
			...
			// Format string after data is loaded
			format(str, sizeof(str), "[ID: %d]\n{ffffff}%s"YELLOW_E"\nPress 'H' to Parkveh/PickupVeh", id, GetLocation(ppData[id][parkX], ppData[id][parkY], ppData[id][parkZ]));
...
// Fixed: Use iterator instead of loop
...
CMD:createpark(playerid, params[])
{
	...
	mysql_format(g_SQL, query, sizeof(query), "INSERT INTO parks SET id=%d, posx='%f', posy='%f', posz='%f', interior=%d, world=%d", id, ppData[id][parkX], ppData[id][parkY], ppData[id][parkZ], ppData[id][parkInt], ppData[id][parkWorld]);
...
CMD:setparkpos(playerid, params[])
{
	...
	Park_Save(id);
	Servers(playerid, "Park Point ID %d position updated.", id);
	return 1;
}
...
CMD:removepark(playerid, params[])
{
	...
	ppData[id][parkLabel] = Text3D: INVALID_3DTEXT_ID;
	ppData[id][parkMap] = -1;

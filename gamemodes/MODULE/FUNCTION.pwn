...
function OnPlayerDataLoaded(playerid, race_check)
{
	...
	cache_get_value_name(0, "email", pData[playerid][pEmail], 65);
    ...
    // Mengisi cache dengan query

	mysql_format(g_SQL, query, sizeof(query), "SELECT IsLoggedIn FROM playerucp WHERE UCP = '%e'", pData[playerid][pUCP]);
	mysql_tquery(g_SQL, query, "OnUpdateLoginStatus", "d", playerid);


    // Memeriksa hasil cache
    if (cache_num_rows() > 0)
    {
        new is_online;
        cache_get_value_name_int(0, "IsLoggedIn", is_online);
        
        if(is_online == 1)
        {
            ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Account Already Logged In", 
                "{FF0000}Akun ini sedang login di device lain!\n{FFFFFF}Hubungi administrator jika ini kesalahan.", "Close", "");
        ...
        {
            new str[400];
            format(str, sizeof(str), "UCP: {15D4ED}%s\n{ffffff}Silahkan masukkan PIN yang telah di kirimkan oleh BOT", pData[playerid][pUCP]);
        ...
        {
            new lstring[512];
            format(lstring, sizeof lstring, "{ffffff}This account is {5efc03}Registered!\n{ffffff}User Control Panel: {15D4ED}%s\n{ffffff}You still have {FFFF00}%i/3 attempt(s){ffffff}\nYou have 60 second, so please input your password", pData[playerid][pUCP], pData[playerid][LoginAttempts]);
            ShowPlayerDialog(playerid, DIALOG_LOGIN, DIALOG_STYLE_PASSWORD, "Welcome to Valencia Roleplay", lstring, "Login", "Abort");
        }
        pData[playerid][LoginTimer] = SetTimerEx("OnLoginTimeout", SECONDS_TO_LOGIN * 1000, false, "i", playerid);
    }
    else
    {
        ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX,"Account Unregistered","{FFFF00}This UCP is not registered!\n{ffffff}Please register on our Official Discord {15D4ED}"SERVER_NAME"\n{ffffff}Discord Link: {15D4ED}discord.gg/xtRmyd9kWG","Close","");
        SetTimerEx("KickTimer", 3000, 0, "i", playerid);
    }
    mysql_format(g_SQL, query, sizeof(query), "SELECT * FROM `banneds` WHERE `name` = '%s' OR `ip` = '%s' OR (`longip` != 0 AND (`longip` & %i) = %i) LIMIT 1", pData[playerid][pUCP], PlayerIP, BAN_MASK, (Ban_GetLongIP(PlayerIP) & BAN_MASK));
	...
	if(cache_num_rows() > 0)
	{
		...
		else
		{
			foreach(new pid : Player)
			{
				...
				{
					SendClientMessageEx(pid, COLOR_WHITE, "(!) "GREY2_E"%s(%i) has been auto-kicked for ban evading.", pData[playerid][pUCP], playerid);
				}
			}
			new query[248], PlayerIP[16];
	...
	if(cache_num_rows() > 0)
	{
		...
		else
		{
			foreach(new pid : Player)
			{
				if(pData[pid][pTogLog] == 0)
				{
					SendClientMessageEx(pid, COLOR_WHITE, "(!) "GREY2_E"%s(%i) has been auto-kicked for ban evading.", pData[playerid][pName], playerid);
				}
			}
			new query[248], PlayerIP[16];
...
function AssignPlayerData(playerid)
{
	...
	cache_get_value_name_int(0, "characterstory", pData[playerid][pPlayerCS]);
	...
	cache_get_value_name_int(0, "trucker_level", pData[playerid][pTruckerLevel]);
	cache_get_value_name_int(0, "trucker_levelup", pData[playerid][pTruckerLevelup]);
	...
	//cache_get_value_name_int(0, "IsLoggedIn", pData[playerid][pIsloggedIn]);
	...
	ClearChat(playerid);
	SM(playerid, ""WHITE_E" {ffff00}<i> {15ed81}Welcome to "SERVER_NAME"!");
	SM(playerid, ""WHITE_E" {ffff00}(i) {ffffff}Dear {15ed81}%s{ffffff}, Happy Roleplay and Please Follow the Rules!", pData[playerid][pName]);
	SM(playerid, ""WHITE_E" {ffff00}(i) {ffffff}Last login: {15ed81}%s",pData[playerid][pLastLogin]);
	SM(playerid, ""WHITE_E" {ffff00}(i) {ffffff}Please read & follow the rules for better roleplay experience");
	SM(playerid, ""WHITE_E" {ffff00}(i) {ffffff}Server needs {15ed81}%d{ffffff}ms to load your character", playerPing);
	SM(playerid, ""WHITE_E" INFO: {FFFFFF}You need tutorial? use '{15ed81}/tutorial{ffffff}' to start");
	SM(playerid, ""WHITE_E" MOTD: {ffffff}%s", MOTD);
	...
	foreach(new ii : Player)
	{
		...
		{
			SendClientMessageEx(ii, COLOR_WHITE, "(!)"SILVER_E" %s connected to the server.", pData[playerid][pName]);
		}
	}
...
function OnPlayerRegister(playerid)
{
	...
	pData[playerid][pMoney] = 50000;
	pData[playerid][pBankMoney] = 100000;

	// Set level default ke 1
    pData[playerid][pTruckerLevel] = 1;
    // Set exp awal ke 0
    pData[playerid][pTruckerLevelup] = 0;
...
function OnOBanQueryData(adminid, NameToBan[], banReason[], banTime)
{
	...
	else
	{
		...
		if(banTime != 0)
	    {
			...
			SendClientMessageToAllEx(COLOR_ADMCMD, "AdmCmd: Admin %s has blocked %s for %d days.", pData[adminid][pAdminname], NameToBan, banTime);
		...
		else
		{
			...
			SendClientMessageToAllEx(COLOR_ADMCMD, "AdmCmd: Admin %s has blocked %s permanently.", pData[adminid][pAdminname], NameToBan);
			SendClientMessageToAllEx(COLOR_ADMCMD, "Reason: %s", banReason);
		}
		new query[512];
...
{
	TogglePlayerControllable(playerid, true); // Enable player control after the freeze
	return 1;
}
...
function CheckPlayerSpawn3Titik(playerid)
{
	...
	{
		SetPlayerPos(playerid, 2935.1484, -2051.7058, 3.5480);
		SetPlayerFacingAngle(playerid, 89.0291);
		SetPlayerInterior(playerid, 0);
		SetPlayerVirtualWorld(playerid, 0);
		pData[playerid][pSpawnList] = 0;		
		ClearAnimations(playerid);	
...
/*SendSMS(playerid, phoneNumber, text[])
{
    foreach(new ii : Player)
    {
        if(pData[ii][pPhone] == phoneNumber)
        {
            if(pData[ii][pPhoneStatus] == 0)
                return Error(playerid, "Tidak dapat SMS, ponsel yang dituju sedang offline.");
            ...
            if(ii == INVALID_PLAYER_ID || !IsPlayerConnected(ii))
                return Error(playerid, "Nomor ini tidak aktif!");

            SendClientMessageEx(playerid, COLOR_YELLOW, "[SMS to %d]"WHITE_E" %s", phoneNumber, text);
    ...
    return Error(playerid, "Nomor telepon tidak ditemukan.");
}*/
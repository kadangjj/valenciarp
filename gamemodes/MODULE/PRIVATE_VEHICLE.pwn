...
Vehicle_GetStatus(carid)
{
    ...
    {
        new 
            panels,      // Integer for vehicle panel status
            doors,       // Integer for vehicle door status
            lights,      // Integer for vehicle light status
            tires;       // Integer for vehicle tire status
        
        // Mengambil status kerusakan kendaraan
        GetVehicleDamageStatus(pvData[carid][cVeh], panels, doors, lights, tires);
        pvData[carid][cDamagePanels] = panels;
        pvData[carid][cDamageDoors] = doors;
        pvData[carid][cDamageLights] = lights;
        pvData[carid][cDamageTires] = tires;

        // Mengambil status kesehatan kendaraan
        GetVehicleHealth(pvData[carid][cVeh], pvData[carid][cHealth]);

        // Mengambil bahan bakar kendaraan
        pvData[carid][cFuel] = GetVehicleFuel(pvData[carid][cVeh]);

        // Mengambil posisi dan sudut kendaraan
...
function OnLoadVehicleStorage(i)
{
	if(IsValidVehicle(pvData[i][cVeh]))
	{
		...
		if(pvData[i][cFish] > 0)
		{
			VehFish[pvData[i][cVeh]] = pvData[i][cFish];
		}
		else
		{
			VehFish[pvData[i][cVeh]] = 0;
		}
...
/*function LoadPV(playerid)
{
	...
	// function SaveVehicleDamage(carid) {
  //   new panels, doors, light, tires;
  //   GetVehicleDamageStatus(carid, panels, doors, light, tires);

  //   // Menyimpan status kerusakan di array data kendaraan
  //   pvData[carid][cDamagePanels] = panels;
  //   pvData[carid][cDamageDoors] = doors;
  //   pvData[carid][cDamageLights] = light;
  //   pvData[carid][cDamageTires] = tires;
	// }
...
Vehicle_Save(vehicleid)
{
	...
	format(cQuery, sizeof(cQuery), "UPDATE vehicle SET locked='%d', insu='%d', claim='%d', claim_time='%d', x='%f', y='%f', z='%f', a='%f', health='%f', panels='%d', doors='%d', lights='%d', tires='%d', fuel='%d', interior='%d', vw='%d', damage0='%d', damage1='%d', damage2='%d', damage3='%d', color1='%d', color2='%d', paintjob='%d', neon='%d', price='%d', model='%d', plate='%d', plate_time='%d', ticket='%d', mod0='%d', mod1='%d', mod2='%d', mod3='%d', mod4='%d', mod5='%d', mod6='%d', mod7='%d', mod8='%d', mod9='%d', mod10='%d', mod11='%d', mod12='%d', mod13='%d', mod14='%d', mod15='%d', mod16='%d', lumber='%d', metal='%d', coal='%d', product='%d', gasoil='%d',component='%d',fish='%d', box='%d', rental='%d', park='%d', broken='%d', trunk='%d' WHERE id='%d'",
		...
		pvData[vehicleid][cDamagePanels],
		pvData[vehicleid][cDamageDoors],
		pvData[vehicleid][cDamageLights],
		pvData[vehicleid][cDamageTires],
		...
		pvData[vehicleid][cComponent],
		pvData[vehicleid][cFish],
...
function RemovePlayerVehicle(playerid)
{
	foreach(new i : PVehicles)
	{
		if(pvData[i][cOwner] == pData[playerid][pID])
		{
			...
			mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`panels`='%d', ", cQuery, pvData[i][cDamagePanels]);
			mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`doors`='%d', ", cQuery, pvData[i][cDamageDoors]);
			mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`lights`='%d', ", cQuery, pvData[i][cDamageLights]);
			mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`tires`='%d', ", cQuery, pvData[i][cDamageTires]);
			...
			mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`fish`=%d,", cQuery, pvData[i][cFish]);
			...
			if(pvData[i][cVeh] != 0)
			{
					printf("Attempting to remove vehicle ID %d for player ID %d", pvData[i][cVeh], playerid);
					
					DisableVehicleSpeedCap(GetPlayerVehicleID(playerid));
					
					if(IsValidVehicle(pvData[i][cVeh])) {
							DestroyVehicle(pvData[i][cVeh]);
							printf("Vehicle ID %d removed successfully.", pvData[i][cVeh]);
					} else {
							printf("Vehicle ID %d is not valid.", pvData[i][cVeh]);
					}
...
function OnVehCreated(playerid, oid, pid, model, color1, color2, Float:x, Float:y, Float:z, Float:a)
{
	...
	pvData[i][cDamagePanels] = 0;
	pvData[i][cDamageDoors] = 0;
	pvData[i][cDamageLights] = 0;
	pvData[i][cDamageTires] = 0;
	pvData[i][cFuel] = 1000;
...
function OnVehStarterpack(playerid, pid, model, color1, color2, Float:x, Float:y, Float:z, Float:a)
{
	...
	Iter_Add(PVehicles, i);
	OnPlayerVehicleRespawn(i);
	Servers(playerid, "Kamu telah mengambil Startepack kota, 1 Sanchez, 150 Gold & "GREEN_E"$500.00");
...
function OnVehRentPV(playerid, pid, model, color1, color2, cost, Float:x, Float:y, Float:z, Float:a, rental)
{
	...
	new str[150];
	format(str,sizeof(str),"[VEH]: %s menyewa kendaraan seharga $500.00 /one days model %s(%d)!", GetRPName(playerid), GetVehicleModelName(model), model);
	LogServer("Property", str);
	pData[playerid][pBuyPvModel] = 0;
	return 1;
}
...
GetVehicleStorage(vehicleid, item)
{
	static const StorageLimit[][] = {
	   //Snack  Sprunk  Medicine  Medkit  Bandage  Seed  Material  Component  Marijuana
	    {30,    30,     50,  	  5,  	  10,  	   500,  1000,  	   1000, 	 100}
	};
...
CMD:buysparepart(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, DIALOG_SPAREPART, DIALOG_STYLE_TABLIST, "Sparepart Shop", "Sparepart\t{3BBD44}$1,000.00\n{ffffff}Vehicle Repair\t( Membutuhkan Sparepart )", "Select", "Close");
	return 1;
}

CMD:vstorage(playerid, params[])
{
    // Cek jika player di dalam kendaraan
    if(IsPlayerInAnyVehicle(playerid)) 
        return Error(playerid, "You must exit from the vehicle.");

    // Dapatkan kendaraan terdekat
    new x = GetNearestVehicleToPlayer(playerid, 3.5, false);
    static carid = -1;

    // Pastikan kendaraan terdekat valid
    if((carid = Vehicle_Nearest(playerid)) != -1)
    {
        // Cek jika kendaraan memiliki storage
        if(IsAVehicleStorage(x))
        {
            // Cek status trunk kendaraan (terbuka atau tertutup)
            if(!GetTrunkStatus(x)) // Jika bagasi tertutup
            {
                return Error(playerid, "You must open the trunk before accessing the storage.");
            }

            // Cek apakah player adalah pemilik kendaraan atau bagian dari faction yang diizinkan
            if(Vehicle_IsOwner(playerid, carid) || pData[playerid][pFaction] == 1)
            {
                foreach(new i: PVehicles)
                {
                    if(x == pvData[i][cVeh])
                    {
                        ...
                        // Cek status penyimpanan kendaraan (apakah sudah dimuat atau belum)
            ...
            else Error(playerid, "This vehicle does not belong to you!");
        }

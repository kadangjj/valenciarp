...
new LastLoginChar[MAX_PLAYERS][MAX_CHARS][50];

function CheckPlayerChar(playerid)
{
	cache_get_value_name_int(0, "extrac", pData[playerid][pExtraChar]);
	new query[256];
	format(query, sizeof(query), "SELECT `username`, `level`, `last_login` FROM `players` WHERE `ucp`='%s' LIMIT %d;", pData[playerid][pUCP], MAX_CHARS + pData[playerid][pExtraChar]);
...
function LoadCharacter(playerid)
{
	...
	for (new i = 0; i < cache_num_rows(); i ++)
	{
		...
		cache_get_value_name(i, "last_login", LastLoginChar[playerid][i]);
		format(pData[playerid][pLastLogin], 128, "%s", LastLoginChar[playerid][i]);
	}
...
ShowCharacterList(playerid)
{
	...
	{
	    format(sgstr, sizeof(sgstr), "{ffffff}%s\t%d\t%s\n", PlayerChar[playerid][i], LevelChar[playerid][i], LastLoginChar[playerid][i]);
		strcat(name, sgstr);
		count++;
	}
	if(count < MAX_CHARS)
		strcat(name, "{15D4ED}Create Character");
	strins(name, "Character Name\tLevel\tLast Login\n", 0);

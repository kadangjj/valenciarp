...
// Tambahkan fungsi ini di bagian stock functions
stock GetProductName(itemid)
{
	new name[64];
	switch(itemid)
	{
		case 0: name = "Snack";
		case 1: name = "Sprunk";
		case 2: name = "Gas Fuel";
		case 3: name = "Bandage";
		case 4: name = "E-Toll Card";
...
Bisnis_Refresh(id)
{
    if(id != -1)
    {
		...
		DoorCP = CreateDynamicCP(bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ],1.0, -1, -1, -1, 10.0);
		DoorCP = CreateDynamicCP(bData[id][bIntposX], bData[id][bIntposY], bData[id][bIntposZ],1.0, -1, -1, -1, 10.0);
...
CMD:sellbusiness(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, DIALOG_SELL_BISNISS, DIALOG_STYLE_LIST, "Sell Business", CMDSString, "Sell", "Cancel");
	return 1;
}

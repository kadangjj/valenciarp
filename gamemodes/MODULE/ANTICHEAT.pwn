	...
	if((AntiCheatKontol) && pData[playerid][pAdmin] < 2)
	{
	    if(!IsPlayerInRangeOfPoint(playerid, 3.0, pData[playerid][pPosX], pData[playerid][pPosY], pData[playerid][pPosZ]))
	    {
		    ...
		    if(pData[playerid][pACWarns] < MAX_ANTICHEAT_WARNINGS)
		    {
	        	...
	        	new dc[128];
				format(dc, sizeof(dc),  "```\n<!> %s is possibly teleport hacking [Distance: %.1f]```", ReturnName(playerid), distance);
				SendDiscordMessage(3, dc);
			}
			else
			{
		    	SendClientMessageToAllEx(COLOR_ADMCMD, "BotCmd: %s has been automatically kicked by %s.", ReturnName(playerid), SERVER_BOT);
				SendClientMessageToAllEx(COLOR_ADMCMD, "Reason: Teleport Hack"); 

		    	new dc[128];
				format(dc, sizeof(dc),  "```\n<!> %s has been automatically kicked by %s\nReason: Teleport Hack```", ReturnName(playerid), SERVER_BOT);
				SendDiscordMessage(3, dc);
		    	KickEx(playerid);
			}
		}
...
public OnPlayerAirbreak(playerid)
{
	if((AntiCheatKontol) && pData[playerid][pAdmin] < 2)
	{
	    ...
	    if(pData[playerid][pACWarns] < MAX_ANTICHEAT_WARNINGS)
	    {
	        ...
	        new dc[128];
			format(dc, sizeof(dc),  "```\n<!> %s is possibly using airbreak hacks.```", ReturnName(playerid));
			SendDiscordMessage(3, dc);
		}
		else
		{
		    ...
		    SendClientMessageToAllEx(COLOR_ADMCMD, "BotCmd: %s has been automatically banned by %s.", ReturnName(playerid), SERVER_BOT);
			SendClientMessageToAllEx(COLOR_ADMCMD, "Reason: Airbreak"); 

		    new dc[128];
			format(dc, sizeof(dc),  "```\n<!> %s has been automatically banned by %s\nReason: Airbreak```", ReturnName(playerid), SERVER_BOT);
			SendDiscordMessage(3, dc);
	...
	if(gettime() > pData[playerid][pACTime])
	{
		...
		if((AntiCheatKontol) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER && GetVehicleSpeed(GetPlayerVehicleID(playerid)) > 350 && pData[playerid][pAdmin] < 2)
		{
		    ...
		    if(pData[playerid][pACWarns] < MAX_ANTICHEAT_WARNINGS)
		    {
	        	...
	        	format(dc, sizeof(dc),  "```\n<!> %s is possibly speedhacking, speed: [%.1f Km/H]```", ReturnName(playerid), GetVehicleSpeed(GetPlayerVehicleID(playerid)));
				SendDiscordMessage(3, dc);
			}
			else
			{
                new dc[128];
                SendClientMessageToAllEx(COLOR_ADMCMD, "BotCmd: %s has been automatically kicked by %s.", ReturnName(playerid), SERVER_BOT);
				SendClientMessageToAllEx(COLOR_ADMCMD, "Reason: Speed hacks");

		    	format(dc, sizeof(dc),  "```\n<!> %s has been automatically kicked by %s\nReason: Speed Hack```", ReturnName(playerid), SERVER_BOT);
				SendDiscordMessage(3, dc);
		    	KickEx(playerid);
			}
		}
		...
		if(GetPlayerSpecialAction(playerid) == SPECIAL_ACTION_USEJETPACK && pData[playerid][pAdmin] < 2 && !pData[playerid][pJetpack])
		{
            new dc[128];
            SendClientMessageToAllEx(COLOR_ADMCMD, "BotCmd: %s has been automatically kicked by %s.", ReturnName(playerid), SERVER_BOT);
			SendClientMessageToAllEx(COLOR_ADMCMD, "Reason: Jetpack hacks"); 

			format(dc, sizeof(dc),  "```\n<!> %s has been automatically kicked by %s\nReason: Jetpack Hack```", ReturnName(playerid), SERVER_BOT);
			SendDiscordMessage(3, dc);
		    KickEx(playerid);
		}
			...
			switch(GetPlayerAnimationIndex(playerid))
			{
			    case 958, 1538, 1539, 1543:
			    {
                    ...
                    if((z > 20.0) && (0.9 <= floatsqroot((vx * vx) + (vy * vy) + (vz * vz)) <= 1.9) && pData[playerid][pAdmin] < 2)
                    {
                        new dc[128];
                        SendClientMessageToAllEx(COLOR_ADMCMD, "BotCmd: %s has been automatically kicked by %s", ReturnName(playerid), SERVER_BOT);
						SendClientMessageToAllEx(COLOR_ADMCMD, "Reason: Flying hacks"); 

                        format(dc, sizeof(dc),  "```\n<!> %s has been automatically kicked by %s\nReason: Flying Hack```", ReturnName(playerid), SERVER_BOT);
                        SendDiscordMessage(3, dc);
                        KickEx(playerid);
					}
				}
  			...
  			if(!(gettime() - pData[playerid][pLastUpdate] > 5))
  			{
				if(floatround(armor) > floatround(pData[playerid][pArmour]) && gettime() > pData[playerid][pACTime] && gettime() > pData[playerid][pArmorTime] && pData[playerid][pAdmin] < 2)
				{
				    ...
				    if(pData[playerid][pACWarns] < MAX_ANTICHEAT_WARNINGS)
				    {
                        ...
                        format(dc, sizeof(dc),  "```\n<!> %s is possibly Armor hacks. (old: %.2f, new: %.2f)```", ReturnName(playerid), pData[playerid][pArmour], armor);
                        SendDiscordMessage(3, dc);
					}
					else
					{
                        new dc[128];
                        SendClientMessageToAllEx(COLOR_ADMCMD, "BotCmd: %s has been automatically kicked by %s.", ReturnName(playerid), SERVER_BOT);
						SendClientMessageToAllEx(COLOR_ADMCMD, "Reason: Armor hacks"); 

                        format(dc, sizeof(dc),  "```\n<!> %shas been automatically kicked by %s\nReason: Armor Hack```", ReturnName(playerid), SERVER_BOT);
                        SendDiscordMessage(3, dc);
                        KickEx(playerid);
					}
				}

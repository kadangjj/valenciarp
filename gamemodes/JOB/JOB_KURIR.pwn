...
CMD:stopcourier(playerid,params[])
{
    ...
    if(Kurir[playerid] == true)
    {
        ...
        Servers(playerid,"Anda telah menghentikan Misi Pengiriman Kurir, Gaji hanya diterima $100.00 ke Pending Salary anda.");
    }
    return 1;
}

CMD:liftbox(playerid,params[])
{
    if(pData[playerid][pJob] != 8)
        return Error(playerid, "Anda bukan Pekerja Kurir");

    if(LagiKerja[playerid] == false)
        return Error(playerid,"Anda belum mengantarkan paket");
    ...
    if(IsAKurirVeh(GetPlayerVehicleID(playerid)))
    {
        if(GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
        {
            ...
            if(IsAKurirVeh(GetPlayerVehicleID(playerid)))
            {
                ...
                SendClientMessage(playerid, 0x76EEC6FF, "* Anda dapat menghentikan misi pengiriman dengan mengetik /stopcourier");
            }
        }
    }
    return 1;

...
CreateJoinProductionPoint()
{
	//JOBS
	new strings[128];
	CreateDynamicPickup(1239, 23, 1218.3816,186.7438,20.2838, -1, -1, -1, 50);
	format(strings, sizeof(strings), "[PRODUCTION JOBS]\n{ffffff}Jadilah Pekerja Production disini\n{7fffd4}/takejob /accept job");
	CreateDynamic3DTextLabel(strings, COLOR_YELLOW, 1218.3816,186.7438,20.2838, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1); // production job
	
	format(strings, sizeof(strings), "[PRODUCTION JOBS]\n{FFFFFF}/createproduct");
	CreateDynamic3DTextLabel(strings, COLOR_YELLOW, 1337.5054, 1570.6724, 10001.4766, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1); // production job
	
	format(strings, sizeof(strings), "[PRODUCTION JOBS]\n{FFFFFF}/createproduct");
	CreateDynamic3DTextLabel(strings, COLOR_YELLOW, 1347.5974, 1574.0264, 10001.4766, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1); // production job
	
	format(strings, sizeof(strings), "[PRODUCTION JOBS]\n{FFFFFF}/createproduct");
	CreateDynamic3DTextLabel(strings, COLOR_YELLOW, 1347.6165, 1578.2180, 10001.4766, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1); // production job
	
	format(strings, sizeof(strings), "[PRODUCTION JOBS]\n{FFFFFF}/createproduct");
	CreateDynamic3DTextLabel(strings, COLOR_YELLOW, 1343.1050, 1570.6831, 10001.4766, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1); // production job

	CreateDynamicPickup(1239, 23, 1362.0577, 1583.6334, 10001.4766, -1, -1, -1, 50);
	format(strings, sizeof(strings), "[PRODUCTION JOBS]\n{ffffff}Warehouse production\n{7fffd4}/sellproduct");
	CreateDynamic3DTextLabel(strings, COLOR_YELLOW, 1362.0577, 1583.6334, 10001.4766, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1); // production job sell product
...
CMD:createproduct(playerid, params[])
{
	...
	{
		if(IsPlayerInRangeOfPoint(playerid, 2.0, 1343.1050, 1570.6831, 10001.4766) || IsPlayerInRangeOfPoint(playerid, 2.0, 1347.5974, 1574.0264, 10001.4766)
		|| IsPlayerInRangeOfPoint(playerid, 2.0, 1337.5054, 1570.6724, 10001.4766) || IsPlayerInRangeOfPoint(playerid, 2.0, 1347.6165, 1578.2180, 10001.4766))
		{
			new type;
...
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 1362.0577, 1583.6334, 10001.4766)) return Error(playerid, "Kamu tidak didekat Warehouse.");
	...
	if(pData[playerid][CarryProduct] == 1)
	{
		...
		Info(playerid, "Anda menjual 10 bahan makanan dengan harga "GREEN_E"$550.00");

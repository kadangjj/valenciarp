...
CreateJoinMechPoint()
{
	//JOBS
	new strings[128];
	CreateDynamicPickup(1239, 23, 1767.3154,-1719.1577,13.6183, -1, -1, -1, 50);
	format(strings, sizeof(strings), "[MECH JOBS]\n{ffffff}Jadilah Pekerja Mekanik disini\n{7fff00}/takejob /accept job");
	CreateDynamic3DTextLabel(strings, COLOR_YELLOW, 1767.3154,-1719.1577,13.6183, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1); // Taxi
}
...
IsAtMech(playerid)
{
	...
	{
		if(IsPlayerInRangeOfPoint(playerid, 15.0, -1713.3325,13.7036,267.3829) || IsPlayerInRangeOfPoint(playerid, 15.0, 1891.1801, -1812.8009, 14.7925) ||
			IsPlayerInRangeOfPoint(playerid, 15.0, -1712.9479,13.7036,267.3829) || IsPlayerInRangeOfPoint(playerid, 15.0, 1871.4244, -1812.8300, 14.7918) ||
			IsPlayerInRangeOfPoint(playerid, 15.0, -1713.1271,13.7036,264.3958) || IsPlayerInRangeOfPoint(playerid, 15.0, 1840.5981, -1798.7760, 14.7366) ||
			IsPlayerInRangeOfPoint(playerid, 15.0, -1712.6487,13.7036,276.1146) || IsPlayerInRangeOfPoint(playerid, 15.0, 1839.8511, -1786.3756, 14.7922)) //Mech Idlewood
		{
			return 1;
		}

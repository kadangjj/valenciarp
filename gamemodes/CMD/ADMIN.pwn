...
CMD:acmds(playerid)
{
	...
	if(pData[playerid][pAdmin] >= 1)
	{
		strcat(line3, "{ffff00}[UTAMA]: {ffffff}/reports, /ar, /dr, /asks, /ans, /nonrpname /moderatorhelp\n\n");
		...
		strcat(line3, "{ff0000}[Volunteer]: {ffffff}/checkmask, /masked\n\n");
	}
	if(pData[playerid][pAdmin] >= 2)
	{
		...
		strcat(line3, "{ff0000}[Helper]: {ffffff}/veh, /destroyveh, /setadminname, /givemoneyall, /checkucp /setcs /unsetcs\n\n");
...
CMD:checkmask(playerid, params[])
{
	if(pData[playerid][pAdmin] < 1)
	    return Error(playerid, "You don't have permission to use this command.");
	new otherid;
	if(sscanf(params, "u", otherid)) return Usage(playerid, "/checkmask <ID/Name>");

    //SendClientMessage(playerid, COLOR_GREY, "-----------------------------------------------------------");
	SendClientMessageEx(playerid, COLOR_ARWIN, "MASK INFO: "YELLOW_E"%s "WHITE_E"(Mask#%d) | UCP "YELLOW_E"%s", pData[otherid][pName], pData[otherid][pMaskID], pData[otherid][pUCP]);
	//SendClientMessageEx(playerid, COLOR_ARWIN, "UCP "YELLOW_E"%s", pData[otherid][pUCP]);
...
CMD:setcs(playerid, params[])
{
	...
	if(pData[otherid][pPlayerCS] != 0) return Error(playerid, "Player tersebut sudah memiliki character story!");
	...
	new query[256];
	mysql_format(g_SQL, query, sizeof(query), "UPDATE players SET characterstory = %i WHERE reg_id = %i", pData[otherid][pPlayerCS], pData[otherid][pID]);
...
CMD:unsetcs(playerid, params[])
{
	if(pData[playerid][pAdmin] < 2) return PermissionError(playerid);
	new otherid;
	if(sscanf(params, "u", otherid)) return Usage(playerid, "/unsetcs <ID/Name>");
	if(pData[otherid][pPlayerCS] != 1) return Error(playerid, "Player tersebut tidak memiliki character story!");
	pData[otherid][pPlayerCS] = 0;
	Info(playerid, "Anda telah unset character story player %s", pData[otherid][pName]);
	Info(otherid, "Admin %s telah menghapus character story kamu!", pData[playerid][pAdminname]);
...
CMD:asay(playerid, params[]) 
{
    ...
    SendClientMessageToAllEx(COLOR_WHITE,"(%s) %s: "WHITE_E"%s", GetStaffRank(playerid), pData[playerid][pAdminname], ColouredText(text));
    return 1;
}
...
CMD:a(playerid, params[])
{
	...
	foreach(new ii : Player) 
	{
		...
		{
			SendClientMessage(ii, COLOR_YELLOW, mstr);	
		}
	}
	if(i != -1)
	{
...
CMD:togooc(playerid, params[])
{
    ...
    if(TogOOC == 0)
    {
        SendClientMessageToAllEx(COLOR_ADMCMD, "AdmCmd: Admin %s has disabled global OOC chat.", pData[playerid][pAdminname]);
        TogOOC = 1;
    }
    else
    {
...
CMD:id(playerid, params[])
{
    ...
    GetPlayerVersion(otherid, clientVersion, sizeof(clientVersion));
...
CMD:caps(playerid, params[])
{
	...
	if(!GetPVarType(otherid, "Caps"))
	{
		...
		SendClientMessageToAllEx(COLOR_ADMCMD, "AdmCmd: Admin %s has disabled the anti-caps feature for player %s", pData[playerid][pAdminname], pData[playerid][pName]);
	}
	else
	{
	    // Enable Caps
		DeletePVar(otherid, "Caps");
		SendClientMessageToAllEx(COLOR_ADMCMD, "AdmCmd: Admin %s has enabled the anti-caps feature for player %s", pData[playerid][pAdminname], pData[playerid][pName]);
	}
	return 1;
}
...
function OWarnPlayer(adminid, NameToWarn[], warnReason[])
{
	...
	else
	{
		...
		SendClientMessageToAllEx(COLOR_ADMCMD, "AdmCmd: Admin %s has given a warning (offline) to player %s.", pData[adminid][pAdminname], NameToWarn);
		SendClientMessageToAllEx(COLOR_ADMCMD, "Reason: %s", warnReason);
...
function OJailPlayer(adminid, NameToJail[], jailReason[], jailTime)
{
	...
	else
	{
		...
		SendClientMessageToAllEx(COLOR_ADMCMD, "AdmCmd: Admin %s has jailed (offline) player %s for %d minutes.", pData[adminid][pAdminname], NameToJail, jailTime);
		SendClientMessageToAllEx(COLOR_ADMCMD, "Reason: %s", jailReason);
...
CMD:unjail(playerid, params[])
{
	...
	SendClientMessageToAllEx(COLOR_ADMCMD, "AdmCmd: Admin %s has been unjailed %s", pData[playerid][pAdminname], pData[otherid][pName]);
	new str[150];
...
CMD:kick(playerid, params[])
{
    ...
    SendClientMessageToAllEx(COLOR_ADMCMD, "AdmCmd: Admin %s has kicked player %s.", pData[playerid][pAdminname], pData[otherid][pName]);
	SendClientMessageToAllEx(COLOR_ADMCMD, "Reason: %s", reason);
  	new str[150];
...
CMD:banucp(playerid, params[])
{
	...
	if(datez != 0)
	{
		SendClientMessageToAllEx(COLOR_ADMCMD, "AdmCmd: Admin %s has blocked UCP %s for %d days.", pData[playerid][pAdminname], pData[otherid][pUCP], datez);
		SendClientMessageToAllEx(COLOR_ADMCMD, "Reason: %s", tmp);

		new dc[500];
		format(dc, sizeof(dc),  "```\nServer: Admin %s has banned ucp %s for %d days. [Reason: %s]", pData[playerid][pAdminname], pData[otherid][pUCP], datez, tmp);
		SendDiscordMessage(3, dc);
	...
	{
		SendClientMessageToAllEx(COLOR_ADMCMD, "AdmCmd: Admin %s has blocked UCP %s permanently.", pData[playerid][pAdminname], pData[otherid][pUCP]);
		SendClientMessageToAllEx(COLOR_ADMCMD, "Reason: %s", tmp);

		new dc[500];
		format(dc, sizeof(dc),  "```\nServer: Admin %s has permanently banned ucp %s.```", pData[playerid][pAdminname], pData[otherid][pUCP], datez, tmp);
		SendDiscordMessage(3, dc);
		new str[150];
...
CMD:obanucp(playerid, params[])
{
    ...
    new NameToBan[60], banReason[128];
    new banTime, datez;
    new PlayerIP[16];
    new query[256];

    // Parsing parameter
    if(sscanf(params, "s[128]d", NameToBan, banTime, banReason))
    {
        Usage(playerid, "/obanucp <UCPName> <time (in days) 0 for permanent> <reason>");
        return true;
    }

    // Query untuk mencari data pemain berdasarkan UCP
    mysql_format(g_SQL, query, sizeof(query), "SELECT ucp FROM playerucp WHERE ucp = '%s'", NameToBan);
    mysql_query(g_SQL, query);

    // Periksa hasil query
    if(!cache_num_rows())
    {
        Error(playerid, "Account '%s' does not exist.", NameToBan);
        return true;
    }

    // Ambil IP dari hasil query
    cache_get_value_index(0, 0, PlayerIP);

    // Jika banTime adalah 0, berarti ban permanen
    if(banTime != 0)
    {
        datez = gettime() + (banTime * 86400);
        Servers(playerid, "You have temp-banned %s (UCP: %s) from the server.", NameToBan, PlayerIP);
        SendClientMessageToAllEx(COLOR_ADMCMD, "AdmCmd: Admin %s has blocked UCP %s for %d days.", pData[playerid][pAdminname], NameToBan, banTime);
		SendClientMessageToAllEx(COLOR_ADMCMD, "Reason: %s", banReason);
    }
    else
    {
        datez = 0; // Ban permanen
        Servers(playerid, "You have permanent-banned %s (IP: %s) from the server.", NameToBan, PlayerIP);
        SendClientMessageToAllEx(COLOR_ADMCMD, "AdmCmd: Admin %s has blocked UCP %s permanently.", pData[playerid][pAdminname], NameToBan);
		SendClientMessageToAllEx(COLOR_ADMCMD, "Reason: %s", banReason);
    }

    // Query untuk menyimpan data ban ke database
    mysql_format(g_SQL, query, sizeof(query), "INSERT INTO banneds(name, ip, admin, reason, ban_date, ban_expire) VALUES ('%s', '%s', '%s', '%s', UNIX_TIMESTAMP(), %d)", NameToBan, PlayerIP, pData[playerid][pAdminname], banReason, datez);
    mysql_tquery(g_SQL, query);

    // Kirim pesan ke Discord jika diperlukan
    new dc[512];
    if(banTime != 0)
        format(dc, sizeof(dc), "```\nServer: Admin %s has temp-banned UCP %s for %d days. [Reason: %s]```", pData[playerid][pAdminname], NameToBan, banTime, banReason);
    else
        format(dc, sizeof(dc), "```\nServer: Admin %s has permanently banned UCP %s. [Reason: %s]```", pData[playerid][pAdminname], NameToBan, banReason);
    SendDiscordMessage(3, dc);
...
function OnUnbanQueryData(adminid, BannedName[])
{
	if(cache_num_rows() > 0)
	{
		...
		SendClientMessageToAllEx(COLOR_ADMCMD, "AdmCmd: Admin %s has unbanned %s from the server.", pData[adminid][pAdminname], BannedName);
		new str[150];
...
function UnbanUCP(adminid, BannedName[])
{
	if(cache_num_rows() > 0)
	{
		...
		SendStaffMessage(COLOR_ADMCMD, "AdmCmd: Admin %s has unbanned %s from the server.", pData[adminid][pAdminname], BannedName);
		new str[150];
...
CMD:warn(playerid, params[])
{
	...
	pData[otherid][pWarn]++;
	SendClientMessageToAllEx(COLOR_ADMCMD, "AdmCmd: Admin %s has given a warning to player %s.", pData[playerid][pAdminname], pData[otherid][pName]);
	SendClientMessageToAllEx(COLOR_ADMCMD, "Reason: %s [Total Warning: %d/20]",  reason, pData[otherid][pWarn]);

    new dc[500];
	format(dc, sizeof(dc),  "```\nServer: Admin %s has given a warning to player %s. [Reason: %s] [Total Warning: %d/20]```", pData[playerid][pAdminname], pData[otherid][pName], reason, pData[otherid][pWarn]);
	SendDiscordMessage(3, dc);
	return 1;
}
...
CMD:unwarn(playerid, params[])
{
    ...
    SendStaffMessage(COLOR_ADMCMD, "AdmCmd: Admin %s has unwarned 1 point to %s's warnings.", pData[playerid][pAdminname], pData[otherid][pName]);
    new str[150];
...
CMD:respawnsapd(playerid, params[])
{
	...
	SendStaffMessage(COLOR_RED, "Admin %s has respawned SAPD vehicles.", pData[playerid][pAdminname]);
	return 1;
}
...
CMD:respawnsags(playerid, params[])
{
	...
	SendStaffMessage(COLOR_RED, "Admin %s has respawned SAGS vehicles.", pData[playerid][pAdminname]);
	return 1;
}
...
CMD:respawnsamd(playerid, params[])
{
	...
	SendStaffMessage(COLOR_RED, "Admin %s has respawned SAMD vehicles.", pData[playerid][pAdminname]);
	return 1;
}
        ...
        if(GetVehicleModel(i))
        {
            ...
            if (((tempposx < radi) && (tempposx > -radi)) && ((tempposy < radi) && (tempposy > -radi)) && ((tempposz < radi) && (tempposz > -radi)))
            {
				if(IsVehicleEmpty(i))
				{
					...
					SendClientMessageToAllEx(COLOR_ADMCMD, "AdmCmd: Admin %s has responded to a nearby vehicle with a radius of %d.", pData[playerid][pAdminname], radi);
					SendClientMessageToAllEx(COLOR_ADMCMD, "Jika kendaraan lumber pribadi anda terkena respawn admin gunakan /mypv untuk meload kembali lumber anda!");
				}
			}
        }
    }
...
CMD:respawnrad(playerid, params[])
{
	...
	new radi;
	if(sscanf(params, "d", radi)) return Usage(playerid, "/respawnrad [radius] | respawn vehicle nearest");
	
	if(radi > 50) return Error(playerid, "Maximal 50 radius");
	RespawnNearbyVehicles(playerid, radi);
	return 1;
}*/
...
CMD:settime(playerid, params[])
{
	...
	SendClientMessageToAllEx(COLOR_ADMCMD, "Server: Admin %s has changed the time to: "YELLOW_E"%d", pData[playerid][pAdminname], time);
...
CMD:setweather(playerid, params[])
{
    ...
    SendClientMessageToAllEx(COLOR_ADMCMD,"Server: Admin %s have changed the weather ID", pData[playerid][pAdminname]);
...
CMD:cd(playerid)
{
	...
	SendClientMessageToAllEx(COLOR_ADMCMD, "AdmCmd: Admin %s has started a global countdown!", pData[playerid][pAdminname]);
	return 1;
}
...
CMD:banip(playerid, params[])
{
	...
	SendClientMessageToAllEx(COLOR_ADMCMD, "AdmCmd: Admin %s has blocked IP address %s.", pData[playerid][pAdminname], playerid, params);
	new str[150];
...
CMD:setname(playerid, params[])
{
	...
	if(!IsPlayerConnected(otherid)) return Error(playerid, "The player has not logged in yet!");
...
// {
// 	new vehid;
// 	if(sscanf(params, "ud", vehid)) return Usage(playerid, "/buyplate [vehid] [custom]");
...
// 				pvData[i][cPlateTime] = gettime() + (99999 * 86400);
...
CMD:kickjob(playerid, params[]) {
    new otherid;
    if(pData[playerid][pAdmin] < 4 && pData[playerid][pFactionModerator] < 1) {
        return PermissionError(playerid);
    }

    if(sscanf(params, "u", otherid)) {
        return Usage(playerid, "/kickjob [playerid/PartOfName]");
    }
    ...
    // Reset data faction player
    pData[otherid][pFaction] = 0;
    pData[otherid][pFactionLead] = 0;
    pData[otherid][pFactionRank] = 0;
		// pData[otherid][pJob] = 0;

    // Mengirim pesan ke admin dan player yang dikeluarkan
    Servers(playerid, "You have removed %s from the faction.", pData[otherid][pName]);
    Servers(otherid, "%s has removed you from the faction.", pData[playerid][pName]);

    // Log aksi di server
    new str[150];
    format(str, sizeof(str), "Admin: %s mengeluarkan %s dari ketua fraksi!", GetRPName(playerid), GetRPName(otherid));
    LogServer("Admin", str);
...
CMD:deleteucp(playerid, params[])
{
    ...
    // Deklarasi variabel
	new nameucp[24];
	if(sscanf(params, "s[24]", nameucp)) 
		return Usage(playerid, "/deleteucp <nama ucp>");
	
	// Informasi kepada player
	Servers(playerid, "UCP '%s' has been deleted.", nameucp);

	// Logging ke server
	new str[150];
	format(str, sizeof(str), "[UCP]: %s menghapus UCP '%s' (database id: %d)!", pData[playerid][pAdminname], nameucp, pData[playerid][pID]);
	LogServer("Admin", str);

    // Query untuk menghapus UCP dari database
	new query[128];
	mysql_format(g_SQL, query, sizeof(query), "DELETE FROM playerucp WHERE ucp = '%e'", nameucp);
	mysql_tquery(g_SQL, query);
...
CMD:setplate(playerid, params[])
{
    ...
    {
        Usage(playerid, "/setplate [vehid] [custom]");
        return 1; // Return 1 to indicate usage message displayed
    }
    ...
    // Validasi panjang plat (misalnya maksimum 7 karakter)
    if (strlen(customPlate) > 7) 
        return Error(playerid, "Custom plate must be 7 characters or less");

    foreach (new i : PVehicles)
		{
				if (vehid == pvData[i][cVeh])
				{
						// Mengatur nomor plat dengan teks kustom
						format(pvData[i][cPlate], 63, "%s", customPlate);
						SetVehicleNumberPlate(pvData[i][cVeh], pvData[i][cPlate]);
						printf("Custom plate to be set: '%s'", customPlate);
						printf("'%s'", pvData[i][cPlate]);

						// Mengatur waktu berlakunya plat (99999 hari ke depan)
						pvData[i][cPlateTime] = gettime() + (99999 * 86400);
				}
		}
...
CMD:givemoney(playerid, params[])
{
	...
	Servers(playerid, "Kamu telah memberikan uang %s(%d) dengan jumlah %s!", pData[otherid][pName], otherid, FormatMoney(strval(totalcash)));
...
CMD:agive(playerid, params[])
{
	...
	if(IsPlayerConnected(playerid)) 
	{
        ...
        if(sscanf(params, "us[24]d", otherid, name, ammount))
		{
			...
			Info(playerid, "Names: bandage, medicine, snack, sprunk, material, component, marijuana, obat, gps, hp");
			return 1;
		}
		...
		else if(strcmp(name,"hp",true) == 0) 
		{
			pData[otherid][pPhone] += ammount;
			Info(playerid, "Anda telah berhasil memberikan HP kepada %s dengan nomor %d.", ReturnName(otherid), ammount);
			Info(otherid, "Admin %s telah berhasil memberikan HP kepada anda dengan nomor %d.", pData[playerid][pAdminname], ammount);
			ApplyAnimation(playerid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
			ApplyAnimation(otherid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
		}
	}
...
CMD:setprice(playerid, params[])
{
	...
	if(!strcmp(name, "materialprice", true))
    {
        ...
        if(price < 0 || price > 500000)
            return Error(playerid, "Kamu harus menspesifi setidaknya dari 0 sampai "GREEN_E"$5,000.00");
	...
	else if(!strcmp(name, "rawcomponent", true))
    {
		new price;
        if(sscanf(string, "d", price))
            return Usage(playerid, "/setprice [rawcomponent] [price]");
...
CMD:setstock(playerid, params[])
{
	...
	else if(!strcmp(name, "rawcomponent", true))
    {
		new stok;
        if(sscanf(string, "d", stok))
            return Usage(playerid, "/setstok [rawcomponent] [stok]");
        ...
        RawComponent = stok;
		Server_Save();
        SendAdminMessage(COLOR_RED, "%s set rawcomponent stok to %d.", pData[playerid][pAdminname], stok);
    }
	else if(!strcmp(name, "rawfish", true))
    {
		new stok;
        if(sscanf(string, "d", stok))
            return Usage(playerid, "/setstok [rawfish] [stok]");
        ...
        RawFish = stok;
		Server_Save();
        SendAdminMessage(COLOR_RED, "%s set rawfish stok to %d.", pData[playerid][pAdminname], stok);
    }
	new str[150];
...
CMD:moderatorhelp(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "{FF0000}Moderator Commands | New Dawn Roleplay", string, "Close","");
	return 1;
}
...
CMD:motd(playerid, params[])
{
    ...
    if(strlen(params) < 1) // Cek jika input kosong
    {
        return Usage(playerid, "Usage: /motd [New MOTD Text]");
    }

    format(MOTD, sizeof(MOTD), "%s", params); // Mengubah isi MOTD
    SM(playerid, "{00ff00}MOTD successfully updated.");
    return 1;
}
